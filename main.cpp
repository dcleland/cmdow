...
int main(int argc, char *argv[])
{
    ...
    case CW:
        ...
        if (hWnd)
            PostMessage(hWnd, WM_COMMAND, WM_CASCADE_WINDOWS, 0);
        break;
    case TV: // tile vertically
        // TileWindows(NULL, MDITILE_VERTICAL, NULL, 0, NULL);
        hWnd = FindWindow("Shell_TrayWnd", "");
        if (hWnd)
            PostMessage(hWnd, WM_COMMAND, WM_TILE_VERTICALLY, 0);
        break;
    case TH: // tile horizontally
        // TileWindows(NULL, MDITILE_HORIZONTAL, NULL, 0, NULL);
        hWnd = FindWindow("Shell_TrayWnd", "");
        if (hWnd)
            PostMessage(hWnd, WM_COMMAND, WM_TILE_HORIZONTALLY, 0);
        break;
    case AT: // alt tab
        AltTab();
        break;
    case MA: // minimize all
        ...
        hWnd = FindWindow("Shell_TrayWnd", "");
        if (hWnd)
            PostMessage(hWnd, WM_COMMAND, WM_MINIMIZE_ALL, 0);
        break;
    ...
    case FS: // switch to full screen mode
        ...
        if (SCDM)
            SCDM(GetStdHandle(STD_OUTPUT_HANDLE), res, &res);
        break;
    case LISTALL:
    ...
    if (!done)
    {
            ...
            while (w)
            {
                ...
                else if (w->hwnd != me)
                {
                    ...
                    if (s >= len)
                    {
                        if (op == 1)
                        {
                            if (!(CompareString(GetThreadLocale(), NORM_IGNORECASE, w->caption, len, args.caption,
                                                len) -
                                  2))
                            {
                                found = TRUE;
                        ...
                        else /*(op == 3)*/
                        {
                            for (s -= len; s >= 0; --s)
                            {
                                ...
                                if (!(CompareString(GetThreadLocale(), NORM_IGNORECASE, w->caption + s, len,
                                                    args.caption, len) -
                                      2))
                                {
                                    found = TRUE;
        ...
        do
        {
            ...
            if (wcount & (!(args.actopts && DONTBLAMEME)))
            {
                break;
            }
            if (args.tasks[0] != LST)
            {
                wcount++; // keep track of windows acted upon, ignore listing a window
            }
            i = 0;
            while (args.tasks[i])
            {
