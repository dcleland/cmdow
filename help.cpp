...
{
    if ((!lstrcmpi("window", cmd)) || (!lstrcmpi("/window", cmd)) || (!lstrcmpi("/T", cmd)) || (!lstrcmpi("/B", cmd)) ||
        (!lstrcmpi("/F", cmd)) || (!lstrcmpi("/P", cmd)))
    {
    ...
    else if ((!lstrcmpi("/TH", cmd)) || (!lstrcmpi("/TV", cmd)) || (!lstrcmpi("/MA", cmd)) || (!lstrcmpi("/UM", cmd)) ||
             (!lstrcmpi("/UW", cmd)) || (!lstrcmpi("/CW", cmd)) || (!lstrcmpi("/AT", cmd)) || (!lstrcmpi("/FS", cmd)) ||
             (!lstrcmpi("/WM", cmd)))
    {
    ...
    else if ((!lstrcmpi("/ACT", cmd)) || (!lstrcmpi("/INA", cmd)) || (!lstrcmpi("/ENA", cmd)) ||
             (!lstrcmpi("/DIS", cmd)) || (!lstrcmpi("/VIS", cmd)) || (!lstrcmpi("/HID", cmd)) ||
             (!lstrcmpi("/MIN", cmd)) || (!lstrcmpi("/MAX", cmd)) || (!lstrcmpi("/RES", cmd)))
    {
    ...
    else if ((!lstrcmpi("caption", cmd)) || (!lstrcmpi("/caption", cmd)) || (!lstrcmpi("/REN", cmd)) ||
             (!lstrcmpi("/MOV", cmd)) || (!lstrcmpi("top", cmd)) || (!lstrcmpi("/top", cmd)) ||
             (!lstrcmpi("left", cmd)) || (!lstrcmpi("/left", cmd)) || (!lstrcmpi("/SIZ", cmd)) ||
             (!lstrcmpi("width", cmd)) || (!lstrcmpi("/width", cmd)) || (!lstrcmpi("height", cmd)) ||
             (!lstrcmpi("/height", cmd)) || (!lstrcmpi("/TOP", cmd)) || (!lstrcmpi("/NOT", cmd)) ||
             (!lstrcmpi("/CLS", cmd)) || (!lstrcmpi("/END", cmd)))
    {
        printf("This form of CMDOW performs specified action(s) on the specified window(s).\n"
               "Some of these commands allow you to manipulate windows in ways not normally\n"
               "possible. Improper use may cause unexpected results and system instability.\n\n"
    ...
    else if ((!lstrcmpi("/RUN", cmd)) || (!lstrcmpi("state", cmd)) || (!lstrcmpi("/state", cmd)) ||
             (!lstrcmpi("file", cmd)) || (!lstrcmpi("/file", cmd)) || (!lstrcmpi("args", cmd)) ||
             (!lstrcmpi("/args", cmd)))
    {
    ...
    {
        printf("CMDOW [Version 1.4.8] Win32 Commandline Window Utility for NT4/2000/XP/7.\n"
