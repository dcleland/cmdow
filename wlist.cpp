...
BOOL CALLBACK GetWindowListProc(HWND hwnd, LPARAM wptr)
{
    ...
    if (w->level == 1)
        EnumChildWindows(hwnd, (WNDENUMPROC)GetWindowListProc, 0);
...
#define MAX_CLASSNAME_LEN 63
    ...
    if (hwnd == desktop)
        w->level = 0;
    else if ((parent == w->owner) || (parent == desktop))
    {
        w->level = 1;
    }
    else
    ...
    {
        GetClassName(hwnd, w->caption, len + 1);
    }
    return TRUE;
}
...
static void LstWinLen(struct WLIST *w, BOOL tb)
{
    ...
    while (w)
    {
        ...
        {
            if (max_handle < w->hwnd)
            {
                max_handle = w->hwnd;
            }
            if (max_level < w->level)
            {
                max_level = w->level;
            }
            if (max_pid < w->pid)
    ...
    len_handle = (DWORD_PTR)max_handle > 0xFFFFFFF ? 8 : (DWORD_PTR)max_handle > 0xFFFFFF ? 7 : 6;
...
{
    printf("%-*s%*s%*s -Window status- ", len_handle + 2, "Handle", len_level + 1, "Lev", len_pid + (len_level != 1),
           "Pid");
    //
    ...
    printf("%-*sCaption\n", fullcapt ? len_image + 1 : showpos && len_pid == 5 ? 8 : 9, "Image");
}
...
void LstWin(struct WLIST *w, struct ARGS *a)
{
    ...
    if ((!headings++))
    {
        ...
        if (!(a->listopts & BARE))
            PrintWindowInfHeadings(a->listopts & SHOWPOS, a->listopts & FULLCAPT);
    }
    ...
    MinMaxRes = (w->styles & WS_MINIMIZE) ? "Min" : (w->styles & WS_MAXIMIZE) ? "Max" : "Res";
    ...
    printf("0x%.*lX %*i %*lu %s %s %s %s ", len_handle, (unsigned long)w->hwnd, len_level, w->level, len_pid,
           (unsigned long)w->pid, MinMaxRes, ActiveInactive, EnabledDisabled, VisibleHidden);
...
{
    if ((!(w->styles & WS_VISIBLE)) || (w->exstyles & WS_EX_TOOLWINDOW) || w->owner || (w->level != 1))
    {
        return FALSE;
    }
    return TRUE;
...
void SetFGWindow(HWND hwnd)
{
    ...
    if (Res && FLT)
    {
        SetForegroundWindow(GetMyHandle());
    }
...
void RenWin(struct WLIST *w, struct ARGS *a)
{
    ...
    // BOOL retval;
    // retval =
    CreateProcess(NULL, cmdline, NULL, NULL, FALSE, CREATE_NO_WINDOW, NULL, NULL, &sInfo, &pi);
    return;
}
...
#define SHELLEXEC_SUCCESS 33
    int RetVal;
    // FindExecutable() Retrieves the name of and handle to the executable (.exe) file associated with the specified
    // file name
    /*STARTUPINFO si;
    PROCESS_INFORMATION pi;
    char buf[1024];
